 ...
 * 此句作用相当天设置可选参数
 */
export function httpGet<T = any>(url: string, params?: any, { showLoading = true }: configInter = {}): Promise<T> {
	const config: configInter = { showLoading }
	return ajax_main('GET', url, params, config)
}
export function httpPost<T = any>(url: string, params?: any, { showLoading = true }: configInter = {}): Promise<T> {
	const config: configInter = { showLoading }
	return ajax_main('POST', url, params, config)
}
export const httpDelete = (url: string, params?: any, { showLoading = true }: configInter = {}) => {
	const config: configInter = { showLoading }
	return ajax_main('DELETE', url, params, config)
}
export const httpPut = (url: string, params?: any, { showLoading = true }: configInter = {}) => {
	const config: configInter = { showLoading }
